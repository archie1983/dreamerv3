...
class Encoder(nj.Module):
  ...
  def __init__(self, obs_space, **kw):
    print("AE: obs_space, kw: ", obs_space, kw)
    ...
    self.kw = kw
    print("AE: vec_keys, img_keys, depths: ", self.veckeys, self.imgkeys, self.depths)
    ...
    if self.imgkeys:
      ...
      for i, depth in enumerate(self.depths):
        ...
        if self.outer and i == 0:
          ...
          x = self.sub(f'cnn{i}', nn.Conv2D, depth, K, **self.kw)(x)
          B, H, W, C = x.shape # presumably: Batch, Height, Width, Colours
